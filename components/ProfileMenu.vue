...
<template>
  <div>
    <Menu as="div" class="relative">
      <div>
        <MenuButton class="flex items-center font-bold text-label-1 focus:outline-none dark:text-label-dark-1">
          {{ $t("nav.profile") }}
      ...
      <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
        ...
        <MenuItems
          ...
          <div class="p-1">
            ...
            }" class="flex w-full items-center justify-between rounded-lg px-3 py-2" @click="(e: MouseEvent) => {
    e.preventDefault();
    autoPlayEnabled = !autoPlayEnabled;
  }
  ">
              <span>Auto Play</span>
              <Switch v-model="autoPlayEnabled" :class="autoPlayEnabled
                ...
                <span aria-hidden="true" :class="autoPlayEnabled ? 'translate-x-full' : 'translate-x-0'
                  "
            ...
            <button :class="{
              'bg-label-separator dark:bg-label-dark-separator': active,
            }" class="w-full rounded-lg px-3 py-2 text-left" @click="(e: MouseEvent) => {
    e.preventDefault();
  }
  ">
              Theme
