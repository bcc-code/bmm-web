...
const windowStore = rendererWindowStore();
const { t } = useI18n();
  ...
  <div v-if="auth0User">
    <DropdownMenu placement="bottom-end">
      <button
        ...
        <FadeInImage v-if="auth0User.picture" :src="auth0User.picture" :alt="auth0User.name || ''"
      ...
      <template #items>
        <DropdownMenuGroup>
          <DropdownMenuItem :title="$t('profile.closeToTray')" @click="(e: MouseEvent) => {
            e.preventDefault();
            windowStore.closeToTray = !windowStore.closeToTray;
          }
            ">
            <Switch v-model="windowStore.closeToTray" :class="windowStore.closeToTray ? 'bg-tint ' : 'bg-background-2 '"
              ...
              <span aria-hidden="true" :class="windowStore.closeToTray ? 'translate-x-full' : 'translate-x-0'
                "
          ...
          <DropdownMenuItem v-if="false" :title="$t('profile.autoplay')" @click="(e: MouseEvent) => {
            e.preventDefault();
            profileStore.autoplay = !profileStore.autoplay;
          }
            ">
            <template #right>
              <Switch v-model="profileStore.autoplay" :class="profileStore.autoplay ? 'bg-tint ' : 'bg-background-2 '"
                class="relative inline-flex h-6 w-10 shrink-0 cursor-pointer rounded-full p-1 transition-colors duration-200 ease-in-out focus:outline-none">
          ...
          <DropdownMenuItem :title="$t('profile.theme')" :secondary-title="getColorModeName(colorMode.preference)"
            @click="showThemeDialog = true" />
          <DropdownMenuItem :title="$t('profile.app-language')"
            :secondary-title="getLocalizedLanguageName(profileStore.uiLanguage)"
            @click="showInterfaceLanguageDialog = true" />
          <DropdownMenuItem :title="$t('profile.content-language')" :secondary-title="getLocalizedList(
        ...
        <DropdownMenuGroup v-if="user && isUploader(user)">
          <DropdownMenuItem title="Admin" href="https://bmm.brunstad.org/admin" target="_blank" />
        </DropdownMenuGroup>
        <DropdownMenuGroup>
          <DropdownMenuItem v-if="!isElectron" :title="$t('download.download')" :to="{ name: 'download' }" />
          <DropdownMenuItem :title="$t('profile.uservoice')" href="https://uservoice.bcc.no/?tags=bmm"
            target="_blank" />
          <DropdownMenuItem :title="$t('profile.contact')" href="mailto:bmm-support@bcc.no" target="_blank" />
          <DropdownMenuItem :title="$t('profile.privacy-policy')" :href="profileStore.uiLanguage === 'nb'
            ? 'https://bcc.media/no/personvern/'
            : 'https://bcc.media/en/privacy/'
            " target="_blank" />
          <DropdownMenuItem :title="$t('track.details.copyright')" :to="{ name: 'copyright' }" />
        </DropdownMenuGroup>
        <DropdownMenuGroup>
    ...
    <DialogBase :show="showThemeDialog" :title="$t('profile.theme')" :description="$t('profile.theme-description')"
      ...
      <RadioGroup v-model="colorMode.preference"
        ...
        <RadioGroupOption v-for="mode in colorModes" :key="mode" :value="mode"
          ...
          <NuxtIcon v-if="mode == colorMode.preference" name="icon.selected"
            class="inline-block text-2xl group-hover:text-4xl" />
        </RadioGroupOption>
      </RadioGroup>
    </DialogBase>

    <DialogBase :show="showInterfaceLanguageDialog" :title="$t('profile.interface-language')"
      :description="$t('profile.interface-language-description')" @close="closeInterfaceLanguageDialog()">
      <div class="rounded-2xl bg-background-2 p-3 text-label-2 md:w-[500px] lg:w-[600px]">
    ...
    <DialogBase :show="showContentLanguageDialog" :title="$t('profile.content-language')"
      :description="$t('profile.content-language-description')" @close="saveAndCloseContentLanguageDialog()">
      <VueDraggable v-model="contentLanguages" handle=".handle" :animation="200"
        class="divide-y divide-label-separator rounded-2xl bg-background-2 md:w-[500px] lg:w-[600px]">
        <div v-for="(item, i) in contentLanguages" :key="item"
          ...
          <button v-if="i > 0" class="text-2xl" @click="contentLanguages.splice(i, 1)">
            <NuxtIcon name="icon.close.small" />
          </button>
        </div>
      </VueDraggable>
      <div v-if="nextUnusedContentLanguage"
        class="type-subtitle-1 mt-4 flex cursor-pointer flex-row gap-2 p-3 text-label-3" @click="() => {
